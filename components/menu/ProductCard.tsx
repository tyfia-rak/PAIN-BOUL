  ...
  return (
    <div className="group w-72 bg-background rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-golden/10 hover:border-golden/30 relative">
      <div className="absolute top-3 left-3 z-10">
        <span className="bg-golden text-charcoal text-xs font-bold px-2 py-1 rounded-full uppercase tracking-wide">
          Nouveau
        </span>
      </div>
      ...
      <div className="p-5 bg-background relative">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="text-2xl font-bold text-golden font-playfair">
            {(product.price ?? 0).toFixed(0)} Ar
          </div>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center bg-background/80 rounded-xl overflow-hidden border border-golden/20 shadow-inner">
            ...
            <span className="w-12 text-center font-inter font-semibold text-foreground">
              {qty}
            </span>
            <button
          ...
          >
            <span
              className={`relative z-10 transition-all duration-300 ${isAdding ? 'opacity-0' : 'opacity-100'}`}
            >
              Ajouter
            </span>
            <span
