...
export default function OrderFormModal({ onClose }: { onClose: () => void }) {
    ...
    const searchCustomer = async () => {
      ...
      try {
        const res = await fetch(
          `${BACKEND_URL}/api/customers/email/${encodeURIComponent(formData.email)}`
        );
        if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validItems = items.filter(
      (item) =>
        item && item.product && item.product.idProduct && item.quantity > 0
    );
          ...
          <div className="mt-6 p-4 bg-background/50 rounded-xl border border-golden/10">
            ...
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {items.map((item, index) => (
                <div
                  key={index}
                  className="flex justify-between items-center font-inter text-sm"
                >
                  <span className="text-foreground">
                    {item.product.name} × {item.quantity}
                  </span>
                  <span className="font-semibold text-golden">
                    {(item.product.price * item.quantity).toFixed(0)} Ar
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block font-inter font-medium text-foreground mb-3 flex items-center gap-2">
                ...
                Email
                {customer && (
                  <span className="ml-2 text-xs text-golden font-normal bg-golden/10 px-2 py-1 rounded-full">
                    ✓ Client existant
                  </span>
                )}
              </label>
              <input
                ...
                className={`w-full px-4 py-3 rounded-lg border bg-background/50 text-foreground placeholder-muted-foreground focus:ring-2 focus:ring-golden/20 transition-all duration-200 font-inter ${
                  customer
                    ...
                    : 'border-golden/20 focus:border-golden'
                }`}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((d) => ({ ...d, deliveryTime: e.target.value }))
                }
          ...
          <div className="border-t border-golden/10 pt-8">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="text-right">
                <span className="font-playfair text-3xl font-bold text-golden">
